...
cv_rmse <- function(p, q, y, x_var, data) {
  ...
  for (train_end in (p + max(q) + 1):(n - 1)) {
    ...
    # Create a matching structure for newdata
    test_data <- data[test_index, , drop = FALSE]
    colnames(test_data) <- colnames(data)  # Ensure column names match
    pred <- as.numeric(predict(model, newdata = test_data))
...
# Check for multicollinearity (optional, to investigate the rank-deficiency warning)
vif_data <- as.data.frame(vif(lm(y ~ ., data = df)))
print(vif_data)

# If necessary, drop variables with high VIF values
